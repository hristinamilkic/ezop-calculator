...
export const generatePDF = (data: CalculatorData) => {
  ...
  pdf.text(`Email: ${companyInfo.email}`, pageWidth - 20, 54, { align: 'right' });
  ...
  data.rows.forEach((row, index) => {
    ...
    const description = row.description.length > 35 
      ? row.description.substring(0, 32) + '...'
      : row.description;
    
    pdf.text(description, colPositions[0], yPosition);
    pdf.text(row.unitPrice.toLocaleString(undefined, { maximumFractionDigits: 2 }), colPositions[1], yPosition);
    pdf.text(row.quantity.toString(), colPositions[2], yPosition);
    pdf.text(row.total.toLocaleString(undefined, { maximumFractionDigits: 2 }), colPositions[3], yPosition);
  ...
  pdf.text(subtotal.toLocaleString(undefined, { maximumFractionDigits: 2 }), 185, yPosition, { align: 'right' });

  yPosition += 8;
  pdf.text(`VAT (${data.vatPercent}%):`, 155, yPosition, { align: 'right' });
  pdf.text(vatAmount.toLocaleString(undefined, { maximumFractionDigits: 2 }), 185, yPosition, { align: 'right' });

  yPosition += 8;
  pdf.setFontSize(12);
  pdf.text('Total:', 155, yPosition, { align: 'right' });
  pdf.text(`${monthlyTotal.toLocaleString(undefined, { maximumFractionDigits: 2 })} ${data.currency}`, 185, yPosition, { align: 'right' });
  ...
  pdf.setFontSize(9);
  pdf.setFont('helvetica', 'normal');
  pdf.text('For any questions regarding this quote, please contact us:', 20, yPosition);
  yPosition += 8;
  ...
  pdf.text('Generated by Professional Contract Calculator', pageWidth / 2, pageHeight - 15, { align: 'center' });
  pdf.text(`Generated on: ${new Date().toLocaleString()}`, pageWidth / 2, pageHeight - 10, { align: 'center' });

  // Save the PDF
  const filename = `contract_calculation_${new Date().toISOString().split('T')[0]}.pdf`;
  pdf.save(filename);
};